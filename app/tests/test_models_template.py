...
def describe_template():
    ...
    def describe_image(expect):
        def it_has_generic_extension_when_absent(template):
            expect(template.image) == Path.cwd() / "templates" / "_test" / "default.img"

        def it_creates_template_directory_automatically():
    ...
    def describe_create(expect):
        @pytest.mark.asyncio
        async def it_downloads_the_image():
        ...
        async def it_handles_invalid_urls(url):
            template = await Template.create(url)
            expect(template.valid) == False

        @pytest.mark.asyncio
        async def it_rejects_non_images():
        ...
        @pytest.mark.asyncio
        async def it_handles_custom_templates_lacking_background():
            url = "http://api.memegen.link/images/custom.png?background"
            template = await Template.create(url)
            expect(template.id) == "_error"

        @pytest.mark.asyncio
        async def it_handles_custom_templates_with_invalid_background():
            url = "http://api.memegen.link/images/custom.png?background=https://https://example.com"
            template = await Template.create(url)
            expect(template.id) == "_error"

        @pytest.mark.asyncio
        async def it_handles_meme_urls():
            url = "http://api.memegen.link/images/fry/test.png"
            template = await Template.create(url)
            expect(template.id) == "fry"

    def describe_check(expect):
        @pytest.mark.asyncio
        async def it_determines_overlay_file_extension():
        ...
        @pytest.mark.asyncio
        async def it_accepts_multiple_urls():
            style = ",".join(
                [
        ...
        @pytest.mark.asyncio
        async def it_accepts_default_style_as_placeholder():
            style = "default,https://www.gstatic.com/webp/gallery/1.jpg"
            template = Template.objects.get("perfection")
            expect(await template.check(style)) == True


def describe_overlay(expect):
    def it_is_visible_inside_start_stop_window():
    ...
    def it_timed_when_window_is_partial():
