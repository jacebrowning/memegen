...
def describe_detail():
    def describe_GET(expect, client):
        @pytest.mark.parametrize("slash", ["", "/"])
        def it_includes_metadata(slash):
        ...
        def it_defaults_to_animated_example_when_available():
        ...
        def it_returns_404_when_missing():
    ...
    def describe_POST(expect, client):
        ...
        def it_accepts_template_id_as_custom_background():
            data = {
                "background": "fry",
                "text_lines[]": ["foo", "bar"],
                "extension": "jpg",
        ...
        def it_redirects_if_requested(id):
